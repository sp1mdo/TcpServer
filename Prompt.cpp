...
void print_text(const char *format, ...)
{
    ...
    write(g_telnet_sock, buffer, strlen(buffer));
}
...
void Prompt::handleKey(void)
{
    ...
    while (1)
    {
        if (telnetToCli.empty() == false)
        {
            x = telnetToCli.pull_front();
            break;
        }
...
void Prompt::parseCommand(void)
{
    ...
    for (size_t i = 0; i < m_Input.size(); i++)
    {
        ...
        if (m_AuxMenu.find(command) != m_AuxMenu.end())
        {
            ...
            {
                print_text("Error: Invalid argument. Input is not a valid number.\n");
            }
            print_text("\n");
            executed = true;
...
int Prompt::tryMatch(void)
{
    ...
    for (auto &element : matches)
    {
        ...
        print_text("\r\t %s\n", std::string(getLastWord(element)).c_str());
    }
...
#ifdef UNIX
    if (m_Prefix.empty() == true)
        print_text("\r%s[%s][%u]%s > %s", CYAN_COLOR, m_Name.c_str(), alloc_count, DEFAULT_COLOR, m_Input.c_str());
    else
        print_text("\r%s[%s][%u] %s/%s%s > %s", CYAN_COLOR, m_Name.c_str(), alloc_count, GREEN_COLOR, m_Prefix.c_str(), DEFAULT_COLOR, m_Input.c_str());
#else
    if (m_Prefix.empty() == true)
        print_text("\r[%s][%u] > %s", m_Name.c_str(), alloc_count, m_Input.c_str());
    else
        print_text("\r[%s][%u] %s / > %s", m_Name.c_str(), alloc_count, m_Prefix.c_str(), m_Input.c_str());
#endif
#else
#ifdef UNIX
    if (m_Prefix.empty() == true)
        print_text("\r%s[%s]%s > %s", CYAN_COLOR, m_Name.c_str(), DEFAULT_COLOR, m_Input.c_str());
    else
        print_text("\r%s[%s] %s/%s%s > %s", CYAN_COLOR, m_Name.c_str(), GREEN_COLOR, m_Prefix.c_str(), DEFAULT_COLOR, m_Input.c_str());
#else
    if (m_Prefix.empty() == true)
        print_text("\r[%s] > %s", m_Name.c_str(), m_Input.c_str());
    else
        print_text("\r[%s] %s / > %s", m_Name.c_str(), m_Prefix.c_str(), m_Input.c_str());
#endif
#endif
